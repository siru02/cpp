...
/*   Updated: 2024/03/08 21:42:20 by hgu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{//전역함수에 의한 연산자 오버로딩이므로 인자2개에 대한 연산 오버로딩이다
	...
	cout << std::endl;
	///////////////////////////
	cout << ref.getRawBits() << std::endl; //float형태로 표현해야한다
	return cout;
}
...
Fixed::Fixed(const int num)//인자 num을 고정소수점형태로
{
	...
	fixedPointVal = num << 8;
	//setRawBits(getRawBits() << 8);//8개의 비트를 왼쪽으로 밀어준다
}
...
Fixed::Fixed(const float num)//float인자 num의 정수부분만 뽑아내서 int형 변수에 저장한다
{
	...
	fixedPointVal = roundf(num * 256); //roundf로 256을 곱한값에 반올림을 수행하여 딱 8개비트만 올라가지않고 9번째 비트를 고려하여 더 정확하게 값을 구한다
...
Fixed& Fixed::operator=(const Fixed &ref) //멤버함수에 의한 연산자 오버로딩(클래스라는 지역에 정의되어있다)
{
	...
	this->setRawBits(ref.getRawBits());//현재 인스턴스에 참조할 객체의 값을 복사한다
	return *this;
